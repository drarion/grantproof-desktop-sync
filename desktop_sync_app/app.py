    ...
    def do_POST(self):
        ...
        if self.path == "/pair":
            ...
            try:
                ...
                pair_payload = json.loads(raw_body.decode("utf-8")) if raw_body else {}
                if isinstance(pair_payload, dict):
                    incoming_org = (
                        pair_payload.get("organizationName")
                        or pair_payload.get("organisationName")
                        or pair_payload.get("workspaceName")
                        or pair_payload.get("ngoName")
                    )
                    if isinstance(incoming_org, str) and incoming_org.strip():
                        app.settings.organization_name = incoming_org.strip()
                        app.settings.save()
                        app.organization_name.set(app.settings.organization_name)
                        app.report_engine = GrantProofReportEngine(Path(app.settings.base_folder), organization_name=app.settings.organization_name)
            except Exception:
                pass
            self._send_json(
                {
                    ...
                    "version": APP_VERSION,
                    "organization_name": app.settings.organization_name,
                }
            )
...
class GrantProofDesktopSyncApp:
    def __init__(self) -> None:
        ...
        self.report_engine = GrantProofReportEngine(Path(self.settings.base_folder), organization_name=self.settings.organization_name)
        self.root = Tk()
        ...
        self.scroll_frame_id: int | None = None
        self.scroll_inner: ttk.Frame | None = None

        self.workstation_name = StringVar(value=self.settings.workstation_name)
        self.organization_name = StringVar(value=self.settings.organization_name)
    ...
    def _configure_styles(self) -> None:
        ...
        style.configure("CardTitle.TLabel", background=CARD, foreground=BLUE, font=("Segoe UI Semibold", 16, "bold"))
        style.configure("Body.TLabel", background=CARD, foreground=TEXT, font=("Segoe UI", 10))
        style.configure("BodyStrong.TLabel", background=CARD, foreground=TEXT, font=("Segoe UI Semibold", 10))
    ...
    def _build_ui(self) -> None:
        ...
        self.scroll_canvas = tk.Canvas(shell, bg=BG, highlightthickness=0, bd=0)
        self.scroll_canvas.grid(row=0, column=0, sticky="nsew")
        y_scroll = ttk.Scrollbar(shell, orient="vertical", command=self.scroll_canvas.yview)
        y_scroll.grid(row=0, column=1, sticky="ns")
        self.scroll_canvas.configure(yscrollcommand=y_scroll.set)

        outer = ttk.Frame(self.scroll_canvas, style="App.TFrame", padding=18)
        outer.columnconfigure(0, weight=1)
        outer.rowconfigure(2, weight=1)
        self.scroll_inner = outer
        self.scroll_frame_id = self.scroll_canvas.create_window((0, 0), window=outer, anchor="nw")
        outer.bind("<Configure>", self._on_content_configure)
        self.scroll_canvas.bind("<Configure>", self._on_canvas_configure)
        self._bind_mousewheel(self.scroll_canvas)

        header_card = self._card(outer, padding=18, style="Header.TFrame")
        ...
        ttk.Label(header_card, text="Compagnon PC local-first pour la réception, la synchronisation et la génération de rapports premium.", style="Sub.TLabel", wraplength=640).grid(row=0, column=1, sticky="w")
        ...
        config_card = self._card(body, padding=18)
        ...
        ttk.Entry(config_card, textvariable=self.workstation_name, style="Modern.TEntry", width=42).grid(row=3, column=0, columnspan=2, sticky="ew", pady=(6, 12))

        ttk.Label(config_card, text="Nom de l’ONG / organisation", style="Body.TLabel").grid(row=4, column=0, columnspan=2, sticky="w")
        ttk.Entry(config_card, textvariable=self.organization_name, style="Modern.TEntry", width=42).grid(row=5, column=0, columnspan=2, sticky="ew", pady=(6, 12))

        ttk.Label(config_card, text="Dossier local GrantProof", style="Body.TLabel").grid(row=6, column=0, columnspan=2, sticky="w")
        ttk.Entry(config_card, textvariable=self.folder_path, style="Modern.TEntry", width=42).grid(row=7, column=0, sticky="ew", pady=(6, 12), padx=(0, 12))
        ttk.Button(config_card, text="Choisir…", command=self.choose_folder, style="Quiet.TButton").grid(row=7, column=1, sticky="ew", pady=(6, 12))

        ttk.Label(config_card, text="Port local", style="Body.TLabel").grid(row=8, column=0, columnspan=2, sticky="w")
        ttk.Entry(config_card, textvariable=self.port, style="Modern.TEntry", width=12).grid(row=9, column=0, sticky="w", pady=(6, 14))

        actions_row_1 = ttk.Frame(config_card, style="Card.TFrame")
        actions_row_1.grid(row=10, column=0, columnspan=2, sticky="ew")
        ...
        actions_row_2.grid(row=11, column=0, columnspan=2, sticky="ew", pady=(10, 0))
        ...
        self.short_status_value.grid(row=12, column=0, columnspan=2, sticky="w", pady=(14, 0))

        pairing_card = self._card(body, padding=18)
        ...
        ttk.Label(pairing_card, text="Le QR de jumelage devient actif dès que le compagnon est démarré. En cas de besoin, utilisez le code manuel.", style="Muted.TLabel", wraplength=460).grid(row=1, column=0, sticky="w", pady=(6, 12))
        ...
        ttk.Label(pairing_card, text="Code manuel", style="BodyStrong.TLabel").grid(row=5, column=0, sticky="w", pady=(12, 6))
        self.manual_code_text = tk.Text(pairing_card, height=1, wrap="word", bg=CARD_ALT, fg=TEXT, relief="flat", borderwidth=0, padx=10, pady=8, font=("Consolas", 9))
        ...
        logs_card = self._card(body, padding=16)
        ...
        self.log_text = tk.Text(log_wrap, height=6, wrap="word", bg=CARD_ALT, fg=TEXT, relief="flat", borderwidth=0, padx=12, pady=10, font=("Segoe UI", 10))
    ...
    def _on_canvas_configure(self, event) -> None:
        if self.scroll_canvas is not None and self.scroll_frame_id is not None:
            self.scroll_canvas.itemconfigure(self.scroll_frame_id, width=event.width)
    ...
    def _refresh_pairing_material(self) -> None:
        ...
        img = ImageOps.contain(img, (236, 236))
        canvas = Image.new("RGB", (252, 252), "white")
    ...
    def save_settings(self) -> None:
        ...
        self.settings.organization_name = self.organization_name.get().strip()
