    ...
    def do_POST(self):
        ...
        if self.path == "/reports/rebuild":
            ...
            try:
                ...
                self._send_json({"ok": True, "project_code": clean_project_code or "all", "report_language": clean_report_language or "auto"})
                return
            except Exception as exc:  # noqa: BLE001
                self._send_json({"error": str(exc)}, status=400)
                return
...
class GrantProofDesktopSyncApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(actions, text="Régénérer rapports", command=lambda: self.rebuild_reports(None)).pack(side="left")
        ...
        ttk.Label(pairing_card, text="Jumelage", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(
            pairing_card,
            text="Scannez ce QR code depuis GrantProof Mobile ou utilisez le code manuel si nécessaire.",
            style="Body.TLabel",
            wraplength=380,
        ...
        self.address_value = ttk.Label(pairing_card, text="", style="Body.TLabel", wraplength=380, justify="center")
        self.address_value.grid(row=3, column=0, pady=(8, 4))
        self.status_value = ttk.Label(pairing_card, text="Serveur arrêté", style="Body.TLabel")
        self.status_value.grid(row=4, column=0, pady=(0, 12))
        ttk.Label(pairing_card, text="Code manuel", style="CardTitle.TLabel").grid(row=5, column=0, sticky="w", pady=(2, 8))
        self.manual_code_text = tk.Text(
            pairing_card,
            height=4,
            wrap="word",
            bg="#fbfcff",
        ...
        manual_actions.grid(row=7, column=0, sticky="w", pady=(10, 0))
        ttk.Button(manual_actions, text="Copier le code", command=self.copy_pairing_code).pack(side="left")
        ttk.Button(manual_actions, text="Copier l’adresse", command=self.copy_server_address).pack(side="left", padx=8)
