...
class PremiumActivityReportBuilder:
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for i, (kind, value) in enumerate(values):
            ...
            p.add_run().add_picture(str(self._white_icon(kind)), width=Inches(0.12))
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._soft_icon(kpi.get('icon', 'people'))), width=Inches(0.29))
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            ...
            p.add_run().add_picture(str(self._sector_icon(sector['icon'])), width=Inches(0.52))
    ...
    def _make_icon(self, kind: str, sector: bool = False, style: str = 'soft') -> Path:
        ...
        d = ImageDraw.Draw(im)
        if sector:
            self._draw_sector_icon_pil(d, kind, (base // 2, base // 2), (*_pil_rgb(BLUE_2), 255), scale=4.4)
        else:
            if style == 'soft':
                d.ellipse((26*scale, 26*scale, base - 26*scale, base - 26*scale), fill=(*_pil_rgb(PALE_BLUE), 255), outline=(*_pil_rgb(BORDER), 255), width=8)
                self._draw_line_icon_pil(d, kind, (base // 2, base // 2), (*_pil_rgb(BLUE_2), 255), scale=4.1)
            elif style == 'badge':
