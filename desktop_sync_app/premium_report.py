...
class PremiumActivityReportBuilder:
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_row_height(bottom.rows[0], Inches(2.02))
        for cell, width in zip(bottom.rows[0].cells, [4.45, 5.15]):
            self._set_cell_width(cell, Inches(width))
            self._cell_margins(cell, top=16, bottom=14, start=28, end=28)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.TOP
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for cell, width in zip(table.rows[0].cells, [8.45, 0.90, 1.65]):
            self._set_cell_width(cell, Inches(width))
            self._shade_cell(cell, BLUE)
            self._cell_margins(cell, top=16, bottom=16, start=34, end=26)
        ...
        self._cell_margins(meta_cell, top=12, bottom=12, start=10, end=18)
        values = [
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._spacer_cell(cell, 4.5)
        p = cell.add_paragraph()
        self._pconf(p, before=0, line=1.05, after=4)
        self._run(p, self._truncate_text(data['summary'], 620), size=8.0)
    ...
    def _build_project_logic(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if activity:
            ...
            p = cell.add_paragraph()
            self._pconf(p, line=1.02, after=3)
            p.paragraph_format.left_indent = Inches(0.22)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=8.0, color=ORANGE)
            self._run(p, self._truncate_text(activity, 86), size=8.0)
        if output:
            p = cell.add_paragraph()
            self._pconf(p, after=2, line=0.96)
            self._run(p, 'Output' if lang == 'fr' else 'Output', bold=True, size=8.0, color=ORANGE)
            p = cell.add_paragraph()
            self._pconf(p, line=1.02, after=0)
            p.paragraph_format.left_indent = Inches(0.22)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=8.0, color=ORANGE)
            self._run(p, self._truncate_text(output, 112), size=8.0)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(2.55), height=Inches(1.66))
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(3.05), height=Inches(1.56))
    ...
    def _report_icon_asset(self, kind: str, style: str = 'plain') -> Path | None:
        ...
        if style == 'badge':
            ...
            d.ellipse((26, 26, 230, 230), outline=(*_pil_rgb(ORANGE), 168), width=8)
            d.ellipse((172, 28, 226, 82), fill=(*_pil_rgb(ORANGE), 255))
