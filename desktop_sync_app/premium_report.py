...
class PremiumActivityReportBuilder:
    ...
    def _configure_docx(self, doc: Document) -> None:
        ...
        styles['Normal'].font.size = Pt(8.0)
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._spacer(doc, 5.5)
        top = doc.add_table(rows=1, cols=3)
        self._table_no_borders(top)
        top.alignment = WD_TABLE_ALIGNMENT.CENTER
        top.autofit = False
        self._set_row_height(top.rows[0], Inches(4.42))
        widths = [2.86, 3.82, 2.92]
        for idx, (cell, width) in enumerate(zip(top.rows[0].cells, widths)):
            self._set_cell_width(cell, Inches(width))
            if idx == 0:
                self._cell_margins(cell, top=18, bottom=10, start=24, end=22)
            elif idx == 1:
                self._cell_margins(cell, top=18, bottom=10, start=92, end=22)
            else:
                self._cell_margins(cell, top=14, bottom=8, start=72, end=14)
        ...
        self._set_row_height(bottom.rows[0], Inches(1.88))
        for idx, (cell, width) in enumerate(zip(bottom.rows[0].cells, [4.45, 5.15])):
            self._set_cell_width(cell, Inches(width))
            if idx == 0:
                self._cell_margins(cell, top=0, bottom=8, start=24, end=20)
            else:
                self._cell_margins(cell, top=0, bottom=8, start=92, end=12)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.TOP
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        table.autofit = False
        self._set_row_height(table.rows[0], Inches(0.92))
        for idx, (cell, width) in enumerate(zip(table.rows[0].cells, [7.66, 1.69, 1.65])):
            self._set_cell_width(cell, Inches(width))
            self._shade_cell(cell, BLUE)
            if idx == 0:
                self._cell_margins(cell, top=16, bottom=16, start=1008, end=30)
            elif idx == 1:
                self._cell_margins(cell, top=16, bottom=16, start=20, end=20)
            else:
                self._cell_margins(cell, top=16, bottom=16, start=20, end=1008)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        title_cell, map_cell, meta_cell = table.rows[0].cells
        p = self._cell_p(title_cell)
        self._pconf(p, after=0, line=0.94)
        title_size = 9.6 if len(data['title']) > 76 else (10.6 if len(data['title']) > 60 else 11.6)
        ...
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.LEFT, after=0)
        if Path(data['header_country_picture']).exists():
            p.add_run().add_picture(str(data['header_country_picture']), width=Inches(0.78))
        self._cell_margins(meta_cell, top=12, bottom=12, start=10, end=0)
        values = [
        ...
        for idx, value in enumerate(values):
            ...
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.RIGHT, after=0, line=0.96)
            self._run(p, value, size=8.0, color=WHITE)

    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        self._shade_cell(cell, 'FAFBFD')
        self._border_cell(cell, BORDER)
        self._section_title(cell, 'APERÇU DE L’ACTIVITÉ' if lang == 'fr' else 'ACTIVITY OVERVIEW', 'activity')
        self._spacer_cell(cell, 5.0)
        p = cell.add_paragraph()
        self._pconf(p, before=0, line=1.02, after=4)
        self._run(p, data['summary'], size=8.0)
        self._spacer_cell(cell, 12)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            self._set_row_height(row, Inches(0.24))
            ico, txt = row.cells
            self._set_cell_width(ico, Inches(0.26))
            self._set_cell_width(txt, Inches(2.34))
            ...
            p = self._cell_p(txt)
            self._pconf(p, line=0.96, after=0)
            self._run(p, label + ' : ', bold=True, size=8.0, color=BLUE_2)
            self._run(p, self._truncate_text(value, 58), size=8.0)

    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'CHIFFRES CLÉS' if lang == 'fr' else 'KEY FIGURES', 'monitoring')
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(1.82))
        for kcell in grid.rows[0].cells:
            self._set_cell_width(kcell, Inches(1.25))
            self._cell_margins(kcell, top=12, bottom=6, start=8, end=8)
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._soft_icon(kpi.get('icon', 'people'))), width=Inches(0.36))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.92)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=14.2, color=BLUE_2)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=0.94)
            self._run(p, self._truncate_text(str(kpi.get('label', '')), 24), size=8.0, color=MUTED)
    ...
    def _build_project_logic(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=4, line=0.96)
        p.add_run().add_picture(str(self._plain_icon('tools')), width=Inches(0.18))
        self._run(p, '  ', size=1)
        self._run(p, title, bold=True, size=8.4, color=BLUE_2)
        self._run(p, '  ●', bold=True, size=8.0, color=ORANGE)
        if activity:
            p = cell.add_paragraph()
            self._pconf(p, after=2, line=0.96)
            self._run(p, 'Activité' if lang == 'fr' else 'Activity', bold=True, size=8.0, color=ORANGE)
            p = cell.add_paragraph()
            self._pconf(p, line=0.98, after=2)
            p.paragraph_format.left_indent = Inches(0.22)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=8.0, color=ORANGE)
            self._run(p, activity, size=8.0)
        if output:
            p = cell.add_paragraph()
            self._pconf(p, after=2, line=0.96)
            self._run(p, 'Output' if lang == 'fr' else 'Output', bold=True, size=8.0, color=ORANGE)
            p = cell.add_paragraph()
            self._pconf(p, line=0.98, after=0)
            p.paragraph_format.left_indent = Inches(0.22)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=8.0, color=ORANGE)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(2.72), height=Inches(1.68))
        sectors = data['sectors'][:2] or [{'key': 'coordination', 'label': 'Coordination', 'icon': 'coordination'}]
        self._spacer_cell(cell, 8)
        self._section_title(cell, 'ALIGNEMENT SECTORIEL' if lang == 'fr' else 'SECTOR ALIGNMENT', 'check')
        self._spacer_cell(cell, 3)
        table = cell.add_table(rows=1, cols=max(1, len(sectors)))
        self._table_no_borders(table)
        table.autofit = False
        for scell, sector in zip(table.rows[0].cells, sectors):
            self._set_cell_width(scell, Inches(1.24))
            ...
            p.add_run().add_picture(str(self._sector_icon(sector['icon'])), width=Inches(0.62))
            p = scell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, line=0.95, after=0)
            self._run(p, sector['label'], size=8.0)

    def _build_highlights(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'FAITS SAILLANTS' if lang == 'fr' else 'HIGHLIGHTS', 'evidence', line_chars='━━━━━━━━━━━━━━━━━━━━')
        for item in data['highlights'][:3]:
            p = cell.add_paragraph()
            self._pconf(p, line=1.02, after=2)
            p.paragraph_format.left_indent = Inches(0.16)
            p.paragraph_format.first_line_indent = Inches(-0.16)
            self._run(p, '• ', bold=True, color=ORANGE, size=8.0)
            self._run(p, self._truncate_text(item, 145), size=8.0)

    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'LOCALISATION' if lang == 'fr' else 'LOCATION', 'location')
        self._spacer_cell(cell, 0)
        layout = cell.add_table(rows=1, cols=2)
        self._table_no_borders(layout)
        layout.autofit = False
        left, right = layout.rows[0].cells
        self._set_cell_width(left, Inches(1.60))
        self._set_cell_width(right, Inches(3.28))
        self._cell_margins(left, top=0, bottom=0, start=0, end=16)
        self._cell_margins(right, top=0, bottom=0, start=0, end=0)
        p = self._cell_p(left)
        self._pconf(p, before=0, after=2)
        self._run(p, 'Localisation de l’activité' if lang == 'fr' else 'Activity location', bold=True, size=8.0, color=BLUE_2)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=0.98)
        self._run(p, self._truncate_text(data['location'], 36), size=8.0, color=DARK)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=0.98)
        if data.get('gps_point'):
            lat, lon, _ = data['gps_point']
            self._run(p, f'GPS : {lat:.4f}, {lon:.4f}', size=8.0, color=MUTED)
        else:
            self._run(p, 'Carte nationale affichée par défaut.', size=8.0, color=MUTED)
        p = self._cell_p(right)
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(3.02), height=Inches(1.56))
    ...
    def _build_annex_pages(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for start in range(0, len(images), 6):
            ...
            self._run(p, subtitle, size=8.2, color=MUTED)
    ...
    def _set_table_indent(self, table, inches: float) -> None:
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '──────────') -> None:
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=2, line=0.95)
        if icon:
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.18))
            self._run(p, '  ', size=1)
        self._run(p, title, bold=True, size=8.8, color=BLUE_2)
        self._run(p, '  ●', bold=True, size=8.0, color=ORANGE)
        self._run(p, '  ', size=1)
        self._run(p, '────────', size=8.0, color='B7C7DB')
