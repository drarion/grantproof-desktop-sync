...
class PremiumActivityReportBuilder:
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_row_height(bottom.rows[0], Inches(2.12))
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        table.autofit = False
        self._set_row_height(table.rows[0], Inches(1.02))
        for idx, (cell, width) in enumerate(zip(table.rows[0].cells, [8.16, 1.13, 1.71])):
            self._set_cell_width(cell, Inches(width))
            self._shade_cell(cell, BLUE)
            if idx == 0:
                self._cell_margins(cell, top=24, bottom=22, start=28, end=18)
            elif idx == 1:
                self._cell_margins(cell, top=14, bottom=14, start=0, end=6)
            else:
                self._cell_margins(cell, top=24, bottom=20, start=8, end=64)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        title_cell, map_cell, meta_cell = table.rows[0].cells
        p = self._cell_p(title_cell)
        self._pconf(p, after=2, line=0.94)
        title_size = 10.8 if len(data['title']) < 68 else (9.8 if len(data['title']) < 90 else 9.0)
        ...
        self._run(p, f"{prefix} : {data['project_name']} ({data['project_code']})", size=7.5, color=WHITE)
        ...
        for idx, value in enumerate(values):
            ...
            self._run(p, value, size=8.2, color=WHITE)
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        summary_size = 8.0
        self._pconf(p, before=5, line=1.00, after=3)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            self._set_row_height(row, Inches(0.34))
            ico, txt = row.cells
            self._set_cell_width(ico, Inches(0.40))
            self._set_cell_width(txt, Inches(2.48))
            ...
            p = self._cell_p(ico)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.22))
            p = self._cell_p(txt)
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(1.55))
        for kcell in grid.rows[0].cells:
            self._set_cell_width(kcell, Inches(1.16))
            self._cell_margins(kcell, top=16, bottom=16, start=12, end=12)
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=10)
            p.add_run().add_picture(str(self._plain_icon(self._kpi_icon_key(kpi))), width=Inches(0.45))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.95)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=19.0, color=BLUE_2)
            p = kcell.add_paragraph()
            ...
            label_text = self._truncate_text(str(kpi.get('label', '')), 24)
            if len(label_text) > 14 and ' ' in label_text:
                parts = label_text.split(' ', 1)
                label_text = parts[0] + '\n' + parts[1]
            self._run(p, label_text, size=8.0, color=DARK)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(3.10), height=Inches(1.78))
    ...
    def _build_highlights(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for item in data['highlights'][:3]:
            p = cell.add_paragraph()
            self._pconf(p, line=1.12, after=3)
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '──────────', underline_width: float = 2.2) -> None:
        ...
        if icon:
            ...
            self._run(p, '  ', size=1)
        self._run(p, title, bold=True, size=9.2, color=BLUE_2)
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=4, line=0.50)
        self._run(p, '─' * max(10, int(underline_width * 10)), size=4.0, color=BLUE_3)
