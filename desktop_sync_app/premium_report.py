...
class PremiumActivityReportBuilder:
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        title_size = 11.8 if len(data['title']) < 68 else (10.4 if len(data['title']) < 90 else 9.4)
        ...
        self._run(p, f"{prefix} : {data['project_name']} ({data['project_code']})", size=8.4, color=WHITE)
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        summary_size = 8.7
        self._pconf(p, before=7, line=1.04, after=4)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            ...
            self._run(p, label, bold=True, size=8.4, color=BLUE_2)
            p = txt.add_paragraph()
            self._pconf(p, line=1.03, after=0)
            self._run(p, value, size=8.4, color=DARK)
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(1.95))
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._plain_icon(self._kpi_icon_key(kpi))), width=Inches(0.58))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.95)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=23.0, color=BLUE_2, font='Liberation Sans Narrow')
            p = kcell.add_paragraph()
            ...
            if len(label_text) > 14 and ' ' in label_text:
                ...
                label_text = parts[0] + '\n' + parts[1]
            self._run(p, label_text, size=9.0, color=DARK)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(3.24), height=Inches(1.88))
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            ...
            p.add_run().add_picture(str(self._sector_icon(icon_key)), width=Inches(0.66))
    ...
    def _report_icon_asset(self, kind: str, style: str = 'plain') -> Path | None:
        ...
        max_box = (232, 232) if icon_key.startswith('model_') else ((188, 188) if style == 'badge' else (212, 212))
        if im.width and im.height:
            scale = min(max_box[0] / im.width, max_box[1] / im.height)
            new_size = (max(1, int(im.width * scale)), max(1, int(im.height * scale)))
            im = im.resize(new_size, Image.Resampling.LANCZOS)
        if style in {'white', 'badge'} and not icon_key.startswith('model_'):
            pixels = im.load()
