...
class PremiumActivityReportBuilder:
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        while len(kpis) < 4:
            ...
            if len(kpis) == 0:
                kpis.append({'kind': 'evidence', 'value': data['quantity']['value'], 'label': data['quantity']['label'], 'icon': data['quantity'].get('icon', 'people')})
            elif 'media' not in kinds:
        ...
        for idx, kpi in enumerate(kpis[:4]):
            ...
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 12), bold=True, size=16.2, color=BLUE_2)
            p = kcell.add_paragraph()
            ...
            self._run(p, self._truncate_text(str(kpi.get('label', '')), 25), size=6.6)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            ...
            p.add_run().add_picture(str(self._sector_icon(sector['icon'])), width=Inches(0.44))
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_cell_width(right, Inches(3.35))
        ...
        self._pconf(p, before=8, after=2)
        self._run(p, 'Localisation de l’activité' if lang == 'fr' else 'Activity location', bold=True, size=7.4, color=BLUE_2)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=1.0)
        self._run(p, self._truncate_text(data['location'], 38), size=7.0, color=DARK)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=1.0)
        if data.get('gps_point'):
            lat, lon, _ = data['gps_point']
            self._run(p, f'Point GPS : {lat:.4f}, {lon:.4f}', size=6.5, color=MUTED)
        else:
            self._run(p, 'Carte pays par défaut - point GPS non renseigné.', size=6.4, color=MUTED)
    ...
    def _build_annex_pages(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for start in range(0, len(images), 6):
            ...
            for idx, path in enumerate(chunk):
                ...
                picture = self._prepare_photo(path, self._temp_dir / f'annex_{start + idx}.png', pic_px, fit='contain') if self._temp_dir else None
                p = self._cell_p(c)
    ...
    def _make_icon(self, kind: str, sector: bool = False) -> Path:
        ...
        if official and official.exists():
            ...
            if not sector:
                ...
                d.ellipse((18, 18, 162, 162), fill=(*_pil_rgb(PALE_BLUE), 255), outline=(*_pil_rgb(BORDER_LIGHT), 255), width=3)
                im.thumbnail((112, 112), Image.Resampling.LANCZOS)
            else:
                im.thumbnail((142, 142), Image.Resampling.LANCZOS)
        ...
        d = ImageDraw.Draw(im)
        if not sector:
            d.ellipse((18, 18, size - 18, size - 18), fill=(*_pil_rgb(PALE_BLUE), 255), outline=(*_pil_rgb(BORDER_LIGHT), 255), width=3)
            self._draw_line_icon_pil(d, kind, (90, 90), (*_pil_rgb(BLUE_2), 255), scale=1.0)
        else:
