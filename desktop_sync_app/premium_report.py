...
class PremiumActivityReportBuilder:
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        widths = [2.90, 3.95, 2.75]
        for idx, (cell, width) in enumerate(zip(top.rows[0].cells, widths)):
            self._set_cell_width(cell, Inches(width))
            if idx == 0:
                self._cell_margins(cell, top=18, bottom=12, start=26, end=26)
            elif idx == 1:
                self._cell_margins(cell, top=18, bottom=12, start=120, end=34)
            else:
                self._cell_margins(cell, top=18, bottom=12, start=130, end=22)
        ...
        for idx, (cell, width) in enumerate(zip(bottom.rows[0].cells, [4.45, 5.15])):
            self._set_cell_width(cell, Inches(width))
            if idx == 0:
                self._cell_margins(cell, top=14, bottom=10, start=26, end=24)
            else:
                self._cell_margins(cell, top=14, bottom=10, start=120, end=20)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.TOP
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for idx, (cell, width) in enumerate(zip(table.rows[0].cells, [8.45, 0.90, 1.65])):
            self._set_cell_width(cell, Inches(width))
        ...
        self._cell_margins(meta_cell, top=12, bottom=12, start=18, end=10)
        values = [
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._section_title(cell, 'APERÇU DE L’ACTIVITÉ' if lang == 'fr' else 'ACTIVITY OVERVIEW', 'implementation')
        self._spacer_cell(cell, 5.5)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            self._set_row_height(row, Inches(0.28))
            ico, txt = row.cells
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'CHIFFRES CLÉS' if lang == 'fr' else 'KEY FIGURES', 'distribution')
    ...
    def _build_project_logic(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        p.add_run().add_picture(str(self._plain_icon('implementation')), width=Inches(0.18))
        self._run(p, '  ', size=1)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(2.50), height=Inches(1.52))
        sectors = data['sectors'][:2] or [{'key': 'coordination', 'label': 'Coordination', 'icon': 'coordination'}]
        title_icon = sectors[0]['icon'] if sectors else 'coordination'
        self._spacer_cell(cell, 6)
        self._section_title(cell, 'ALIGNEMENT SECTORIEL' if lang == 'fr' else 'SECTOR ALIGNMENT', title_icon)
    ...
    def _build_highlights(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'FAITS SAILLANTS' if lang == 'fr' else 'HIGHLIGHTS', 'success', line_chars='━━━━━━━━━━━━━━━━━━━━')
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'LOCALISATION' if lang == 'fr' else 'LOCATION', 'gps')
        self._spacer_cell(cell, 3)
        layout = cell.add_table(rows=1, cols=2)
        self._table_no_borders(layout)
        layout.autofit = False
        left, right = layout.rows[0].cells
        self._set_cell_width(left, Inches(1.55))
        self._set_cell_width(right, Inches(3.20))
        ...
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(2.95), height=Inches(1.52))
