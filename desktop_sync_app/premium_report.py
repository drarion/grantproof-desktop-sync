...
class PremiumActivityReportBuilder:
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for icon, label, value in rows:
            ...
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.20))
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(2.18))
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._soft_icon(kpi.get('icon', 'people'))), width=Inches(0.42))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.9)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=15.2, color=BLUE_2)
            p = kcell.add_paragraph()
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '──────────') -> None:
        ...
        if icon:
            ...
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
            p.add_run().add_picture(str(self._badge_icon(icon)), width=Inches(0.22))
        p = self._cell_p(table.cell(0, 1))
        self._pconf(p, after=0, line=0.95)
        self._run(p, title, bold=True, size=8.7, color=BLUE_2)
