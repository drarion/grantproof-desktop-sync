...
INDICATOR_PATTERNS = [
    ('beneficiary', r'(\d[\d\s.,]*)\s*(?:bénéficiaires?|beneficiaires?|beneficiaries|participants?|personnes?)', 'Bénéficiaires', 'Beneficiaries', 'people', 120),
    ('shelter', r'(?:construction|construit|construits|réhabilitation|rehabilitation)?\s*(?:de\s*)?(\d[\d\s.,]*)\s*(?:abris?|shelters?)', 'Abris', 'Shelters', 'shelter_kpi', 115),
    ('kit', r'(\d[\d\s.,]*)\s*(?:kits?)', 'Kits', 'Kits', 'box', 110),
    ('farmer', r'(\d[\d\s.,]*)\s*(?:agriculteurs?|farmers?)', 'Agriculteurs', 'Farmers', 'agriculture_kpi', 105),
    ('household', r'(\d[\d\s.,]*)\s*(?:ménages?|menages?|households?)', 'Ménages', 'Households', 'household', 100),
    ('latrine', r'(\d[\d\s.,]*)\s*(?:latrines?)', 'Latrines', 'Latrines', 'wash_kpi', 95),
    ('school', r'(\d[\d\s.,]*)\s*(?:écoles?|ecoles?|schools?)', 'Écoles', 'Schools', 'education_kpi', 90),
    ('session', r'(\d[\d\s.,]*)\s*(?:formations?|sessions?|ateliers?)', 'Sessions', 'Sessions', 'training', 85),
]
...
class PremiumActivityReportBuilder:
    ...
    def _prepare_data(self, project_code: str, project_name: str, records: list[Any], lang: str) -> dict[str, Any]:
        ...
        indicators = self._extract_indicators(records, sectors, lang)
        ...
        summary = self._narrative(activity, project_name, description, indicators, location, lang)
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        top.autofit = False
        self._set_row_height(top.rows[0], Inches(4.08))
        widths = [3.20, 4.10, 3.40]
        ...
        self._set_row_height(bottom.rows[0], Inches(2.62))
        for cell, width in zip(bottom.rows[0].cells, [5.00, 5.70]):
            self._set_cell_width(cell, Inches(width))
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._run(p, f"●  {self._truncate_text(data['org_name'], 22)}", size=8.3, color=WHITE)
        p = meta_cell.add_paragraph()
        self._pconf(p, line=1.0, after=1)
        self._run(p, f"⌖  {self._truncate_text(data['location'], 22)}", size=8.3, color=WHITE)
        p = meta_cell.add_paragraph()
    ...
    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for icon, label, value in rows:
            ...
            self._pconf(p, line=1.0, after=0)
            self._run(p, self._truncate_text(value, 46), size=6.9)
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        while len(kpis) < 4:
            if len(kpis) == 0:
                kpis.append({'value': data['quantity']['value'], 'label': data['quantity']['label'], 'icon': data['quantity'].get('icon', 'people')})
            elif len(kpis) == 1:
                kpis.append({'value': str(data['media_count']), 'label': 'Médias liés' if lang == 'fr' else 'Linked media', 'icon': 'media'})
            elif len(kpis) == 2:
                kpis.append({'value': str(data['evidence_count']), 'label': 'Preuve consolidée' if lang == 'fr' else 'Consolidated evidence', 'icon': 'check'})
            else:
                kpis.append({'value': data['country_display'], 'label': 'Pays' if lang == 'fr' else 'Country', 'icon': 'pin'})
        grid = cell.add_table(rows=2, cols=2)
        self._table_no_borders(grid)
        grid.autofit = False
        for r in grid.rows:
            self._set_row_height(r, Inches(1.45))
            for c in r.cells:
                self._set_cell_width(c, Inches(1.92))
                self._cell_margins(c, top=36, bottom=24, start=18, end=18)
                self._shade_cell(c, WHITE)
        ...
        for idx, kpi in enumerate(kpis[:4]):
            ...
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 11), bold=True, size=17.2, color=BLUE_2)
            p = kcell.add_paragraph()
            ...
            self._run(p, self._truncate_text(str(kpi.get('label', '')), 23), size=6.8)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(3.14))
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            self._set_cell_width(scell, Inches(1.56))
            self._cell_margins(scell, top=8, bottom=0, start=8, end=8)
            self._shade_cell(scell, WHITE)
            self._border_cell(scell, BORDER_LIGHT)
            p = self._cell_p(scell)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=2)
            p.add_run().add_picture(str(self._sector_icon(sector['icon'])), width=Inches(0.42))
            p = scell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, line=1.0, after=0)
            self._run(p, sector['label'], size=6.9, color=DARK)
    ...
    def _build_highlights(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for item in data['highlights'][:4]:
            p = cell.add_paragraph()
            self._pconf(p, line=1.05, after=1)
            p.paragraph_format.left_indent = Inches(0.16)
            p.paragraph_format.first_line_indent = Inches(-0.16)
            self._run(p, '• ', bold=True, color=BLUE_2, size=8.2)
            self._run(p, item, size=6.9)
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_cell_width(left, Inches(2.00))
        self._set_cell_width(right, Inches(3.55))
        self._cell_margins(left, top=0, bottom=0, start=0, end=40)
        self._cell_margins(right, top=0, bottom=0, start=0, end=0)
        self._section_title(left, 'LOCALISATION' if lang == 'fr' else 'LOCATION', 'pin', line_chars='━━━━━━')
        p = left.add_paragraph()
        self._pconf(p, before=12, after=2)
        self._run(p, 'Localisation de l’activité' if lang == 'fr' else 'Activity location', bold=True, size=7.3, color=BLUE_2)
        p = left.add_paragraph()
        self._pconf(p, after=1, line=1.0)
        self._run(p, self._truncate_text(data['location'], 42), size=7.0, color=DARK)
        p = left.add_paragraph()
        self._pconf(p, after=1, line=1.0)
        sector_labels = ', '.join([s['label'] for s in data['sectors'][:2]])
        self._run(p, ('Secteurs : ' if lang == 'fr' else 'Sectors: ') + sector_labels, size=6.8, color=MUTED)
        if data.get('gps_point'):
            lat, lon, _ = data['gps_point']
            p = left.add_paragraph()
            self._pconf(p, after=0, line=1.0)
            self._run(p, f'GPS : {lat:.4f}, {lon:.4f}', size=6.6, color=MUTED)
        p = self._cell_p(right)
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(3.32))
        p = right.add_paragraph()
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        caption = data['map_location_label'] if data.get('gps_point') else data['country_display']
        self._run(p, caption, bold=True, size=7.6, color=BLUE_2)
    ...
    def _build_annex_pages(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for start in range(0, len(images), 4):
            ...
            self._run(p, title, bold=True, size=15.4, color=BLUE)
            p = doc.add_paragraph()
            self._pconf(p, after=6)
            subtitle = 'Sélection automatique des photos complémentaires liées à l’activité. Les visuels sont conservés en intégralité pour éviter les cadrages agressifs.' if lang == 'fr' else 'Automatically selected additional photos linked to the activity. Visuals are preserved as fully as possible to avoid aggressive cropping.'
            self._run(p, subtitle, size=8.1, color=MUTED)
            ...
            for row in grid.rows:
                self._set_row_height(row, Inches(3.55))
                for cell in row.cells:
                    self._set_cell_width(cell, Inches(5.18))
                    self._shade_cell(cell, SOFT_BLUE)
                    self._border_cell(cell, BORDER_LIGHT)
                    self._cell_margins(cell, top=28, bottom=20, start=24, end=24)
                    cell.vertical_alignment = WD_ALIGN_VERTICAL.TOP
            for idx, path in enumerate(chunk):
                c = grid.cell(idx // 2, idx % 2)
                picture = self._prepare_photo(path, self._temp_dir / f'annex_{start + idx}.png', (980, 640), fit='contain') if self._temp_dir else None
                p = self._cell_p(c)
                self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=4)
                if picture and Path(picture).exists():
                    p.add_run().add_picture(str(picture), width=Inches(4.65))
                p = c.add_paragraph()
                self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, line=1.0, after=0)
                label = f"Média complémentaire {start + idx + 1}" if lang == 'fr' else f'Additional media {start + idx + 1}'
                self._run(p, label, bold=True, size=7.4, color=BLUE_2)
                p = c.add_paragraph()
                self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, line=1.0, after=0)
                self._run(p, f"{data['location']} | {data['report_date']}", size=6.6, color=MUTED)
    ...
    def _make_icon(self, kind: str, sector: bool = False) -> Path:
        ...
        if sector:
            official = self.assets_dir / 'humanitarian' / HUMANITARIAN_ICON_MAP.get(kind, '')
            if official.exists():
                ...
                canvas.alpha_composite(im, ((180 - im.width) // 2, (180 - im.height) // 2))
                canvas.save(path)
        ...
        if sector:
            ...
            d = ImageDraw.Draw(im)
            d.rounded_rectangle((16, 16, size - 16, size - 16), radius=36, fill=(*_pil_rgb(SOFT_BLUE), 255), outline=(*_pil_rgb(BORDER_LIGHT), 255), width=3)
            self._draw_sector_icon_pil(d, kind, (90, 90), (*_pil_rgb(BLUE_2), 255), scale=1.0)
        else:
            im = Image.new('RGBA', (size, size), (255, 255, 255, 0))
            d = ImageDraw.Draw(im)
            d.ellipse((18, 18, size - 18, size - 18), fill=(*_pil_rgb(BLUE_2), 255))
            self._draw_line_icon_pil(d, kind, (90, 90), (*_pil_rgb(WHITE), 255), scale=1.0)
        im.save(path)
        self._icon_cache[key] = path
        return path
    ...
    def _prepare_photo(self, source: Path | None, output: Path, size: tuple[int, int], fit: str = 'cover') -> Path | None:
        ...
        txt = 'Visuel non disponible'
        tw = self._text_w(d, txt, font)
        d.text(((size[0] - tw) / 2, size[1] / 2 - 15), txt, fill='#6B7280', font=font)
        im.save(output, quality=92)
        return output

    def _contain_pad(self, img: Image.Image, size: tuple[int, int]) -> Image.Image:
        img = img.convert('RGB')
        contained = ImageOps.contain(img, size, Image.Resampling.LANCZOS)
    ...
    def _render_map_image(self, data: dict[str, Any], output: Path, size: tuple[int, int], lang: str) -> Path:
        ...
        shape = self._country_shape(data['country'])
        if not shape:
            draw.rounded_rectangle((2, 2, width - 3, height - 3), radius=20, outline='#D4DAE3', fill='#FFFFFF', width=3)
            self._center_text(draw, data['country_display'], (0, 0, width, height), self._font(32, bold=True), '#6B7280')
        ...
        bbox_expanded = (minx - dx * 0.24, miny - dy * 0.22, maxx + dx * 0.24, maxy + dy * 0.22)
        mapper = self._map_transform(bbox_expanded, width, height, 30)
        ...
        for sr in self._iter_shapes():
            ...
            fill = '#DDEAFB' if is_target else '#F0F3F8'
            outline = '#7AA2DA' if is_target else '#D6DEE9'
            for poly in s_polys:
                pts = [mapper(lon, lat) for lon, lat in poly]
                if len(pts) >= 3:
        ...
        draw.text((cx - self._text_w(draw, label, country_font) / 2, cy - 18), label, fill='#5D6B7C', font=country_font)
        ...
        for name, lon, lat in labels[:6]:
            lx, ly = mapper(lon, lat)
            txt = self._display_country(name, name, lang).upper()
            draw.text((lx - self._text_w(draw, txt, label_font) / 2, ly - 8), txt, fill='#A3ACB8', font=label_font)
        point = data.get('gps_point')
        if point:
    ...
    def _polish_source_description(self, description: str) -> str:
        ...
        fixes = [
            (r'\bkits agricole\b', 'kits agricoles'),
            (r'\bkit agricole\b', 'kit agricole'),
            (r'\bgroupement et formations\b', 'groupements et formations'),
            (r'\bgroupement agricole\b', 'groupement agricole'),
            (r'\bgroupements agricole\b', 'groupements agricoles'),
            (r'\bagriculteur au sein\b', 'agriculteurs au sein'),
        ]
    ...
    def _extract_indicators(self, records: list[Any], sectors: list[dict[str, str]], lang: str) -> list[dict[str, str]]:
        corpus = ' '.join([
        ...
        for key, pattern, fr_label, en_label, icon, priority in INDICATOR_PATTERNS:
            for match in re.finditer(pattern, corpus, flags=re.IGNORECASE):
                ...
                if sig in seen:
                    continue
                indicators.append({'kind': key, 'value': number, 'label': label, 'icon': icon, 'priority': str(priority)})
                seen.add(sig)
        indicators.sort(key=lambda item: (-int(item.get('priority', '0')), item['label']))
        indicators = indicators[:2]
        indicators.append({'kind': 'media', 'value': str(len(self._all_media(records))), 'label': 'Médias liés' if lang == 'fr' else 'Linked media', 'icon': 'media', 'priority': '20'})
        indicators.append({'kind': 'evidence', 'value': str(len([r for r in records if getattr(r, 'kind', '') == 'evidence']) or len(records)), 'label': 'Preuve consolidée' if lang == 'fr' else 'Consolidated evidence', 'icon': 'check', 'priority': '10'})
        return indicators[:4]

    def _extract_primary_quantity(self, records: list[Any], sectors: list[dict[str, str]], lang: str) -> dict[str, str]:
        indicators = self._extract_indicators(records, sectors, lang)
        for item in indicators:
            if item.get('kind') not in {'media', 'evidence'}:
                return item
        return indicators[0] if indicators else {'value': '1', 'label': 'Preuves' if lang == 'fr' else 'Evidence items', 'icon': 'check'}
    ...
    def _target_group_label(self, quantity: dict[str, str], sectors: list[dict[str, str]], description: str, lang: str) -> str:
        ...
        if lang == 'fr':
            if 'bénéficiaire' in corpus or 'participant' in corpus:
                return 'Bénéficiaires directs de l’activité'
            if 'agriculteur' in corpus or any(s['key'] == 'agriculture' for s in sectors):
                return 'Agriculteurs, groupements et ménages ciblés'
        ...
        if 'beneficiar' in corpus or 'participant' in corpus:
            return 'Direct beneficiaries of the activity'
        if 'farmer' in corpus or any(s['key'] == 'agriculture' for s in sectors):
            return 'Farmers, groups and targeted households'
    ...
    def _indicator_highlight_text(self, indicators: list[dict[str, str]], lang: str) -> str:
        focus = [item for item in indicators if item.get('kind') not in {'media', 'evidence'}][:2]
        if not focus:
            return ''
        if lang == 'fr':
            return ' et '.join([f"{item['value']} {item['label'].lower()}" for item in focus])
        return ' and '.join([f"{item['value']} {item['label'].lower()}" for item in focus])

    def _narrative(self, activity: str, project_name: str, description: str, indicators: list[dict[str, str]], location: str, lang: str) -> str:
        act = activity[:1].lower() + activity[1:] if activity else ('activité terrain' if lang == 'fr' else 'field activity')
        indicator_text = self._indicator_highlight_text(indicators, lang)
        if lang == 'fr':
            parts = [f"Cette activité de {act} a été documentée à {location} dans le cadre du projet {project_name}."]
            if indicator_text:
                parts.append(f"Les informations disponibles mettent en avant {indicator_text} comme résultats immédiatement visibles de l’activité.")
            if description:
                parts.append(f"La description source mentionne {description}.")
            parts.append("Les éléments collectés permettent d’illustrer la mise en œuvre, de renforcer la traçabilité et de soutenir un reporting bailleur plus solide, sans extrapoler au-delà des informations disponibles.")
        ...
        if indicator_text:
            parts.append(f"Available information highlights {indicator_text} as the most visible immediate results of the activity.")
        if description:
            parts.append(f"The source description states {description}.")
        parts.append('The collected elements help illustrate implementation, strengthen traceability, and support stronger donor reporting without extrapolating beyond available information.')
        return ' '.join(_sentence(p) for p in parts)
    ...
    def _highlights(self, activity: str, project_code: str, description: str, indicators: list[dict[str, str]], media_count: int, location: str, sectors: list[dict[str, str]], lang: str) -> list[str]:
        ...
        if lang == 'fr':
            ...
            if indicator_text:
                highlights.append(f"Les informations source permettent de retenir {indicator_text} comme chiffres clés de l’activité.")
            if media_count > 0:
            ...
            if sector_text:
                highlights.append(f"L’activité s’inscrit principalement dans les secteurs {sector_text.lower()}.")
        ...
        if indicator_text:
            highlights.append(f"Source information highlights {indicator_text} as the main figures for this activity.")
        if media_count > 0:
            highlights.append(f"The evidence is supported by {media_count} linked media asset(s), useful for accountability and results illustration.")
        if sector_text:
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '━━━━━━━━━━━━') -> None:
        ...
        table.autofit = False
        self._set_cell_width(table.cell(0, 0), Inches(0.34))
        self._set_cell_width(table.cell(0, 1), Inches(2.60))
        self._cell_margins(table.cell(0, 0), top=0, bottom=0, start=0, end=20)
        self._cell_margins(table.cell(0, 1), top=0, bottom=0, start=0, end=0)
        if icon:
            p = self._cell_p(table.cell(0, 0))
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
            p.add_run().add_picture(str(self._icon(icon)), width=Inches(0.27))
    ...
    def _draw_line_icon_pil(self, draw: ImageDraw.ImageDraw, icon: str, center: tuple[int, int], color: tuple[int, int, int, int], scale: float = 1.0) -> None:
        ...
        elif icon in {'box', 'kits'}:
            draw.rounded_rectangle((x - s*.42, y - s*.30, x + s*.42, y + s*.26), radius=10, outline=c, width=w)
            draw.line((x - s*.42, y - s*.05, x + s*.42, y - s*.05), fill=c, width=max(2, w-1))
            draw.line((x, y - s*.30, x, y + s*.26), fill=c, width=max(2, w-1))
            draw.line((x - s*.22, y - s*.48, x, y - s*.30, x + s*.22, y - s*.48), fill=c, width=w)
        elif icon in {'household'}:
            draw.line((x - s*.52, y + s*.02, x, y - s*.42, x + s*.52, y + s*.02), fill=c, width=w)
            draw.rectangle((x - s*.35, y + s*.02, x + s*.35, y + s*.46), outline=c, width=w)
        elif icon in {'training'}:
            draw.rectangle((x - s*.45, y - s*.32, x + s*.12, y + s*.32), outline=c, width=w)
            draw.line((x + s*.22, y - s*.24, x + s*.48, y, x + s*.22, y + s*.24), fill=c, width=w)
            draw.ellipse((x - s*.62, y - s*.12, x - s*.46, y + s*.04), outline=c, width=max(2, w-1))
        elif icon in {'agriculture_kpi'}:
            self._draw_sector_icon_pil(draw, 'agriculture', center, c, scale)
        elif icon in {'shelter_kpi'}:
            self._draw_sector_icon_pil(draw, 'shelter', center, c, scale)
        elif icon in {'wash_kpi'}:
            self._draw_sector_icon_pil(draw, 'wash', center, c, scale)
        elif icon in {'education_kpi'}:
            self._draw_sector_icon_pil(draw, 'education', center, c, scale)
