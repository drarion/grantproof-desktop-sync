...
class PremiumActivityReportBuilder:
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        kpi_table = cell.add_table(rows=1, cols=3)
        self._table_no_borders(kpi_table)
        kpi_table.autofit = False
        for kcell in kpi_table.rows[0].cells:
            self._set_cell_width(kcell, Inches(1.24))
            self._cell_margins(kcell, top=72, bottom=62, start=20, end=20)
        ...
        kpis = [
            ...
            ('check', str(data['evidence_count']), 'Preuve consolidée' if lang == 'fr' else 'Consolidated evidence'),
        ]
        for kcell, (icon, value, label) in zip(kpi_table.rows[0].cells, kpis):
            p = self._cell_p(kcell)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=6)
            p.add_run().add_picture(str(self._icon(icon)), width=Inches(0.44))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=0.9)
            self._run(p, value, bold=True, size=20, color=BLUE_2)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=0.8)
            self._run(p, '—', bold=True, size=9, color=ORANGE)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=1.0)
            self._run(p, label, size=7.0)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            ...
            self._cell_margins(scell, top=10, bottom=0, start=10, end=10)
            ...
            self._run(p, sector['label'], size=7.0)
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_cell_width(left, Inches(2.35))
        self._set_cell_width(right, Inches(2.85))
        self._cell_margins(left, top=0, bottom=0, start=0, end=50)
        ...
        self._pconf(p, before=12, after=0)
        self._run(p, 'Localisation de l’activité' if lang == 'fr' else 'Activity location', bold=True, size=7.4, color=BLUE_2)
        p = left.add_paragraph()
        self._pconf(p, after=0)
        self._run(p, '—', bold=True, size=10, color=ORANGE)
        p = self._cell_p(right)
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(2.70))
        p = right.add_paragraph()
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        caption = data['map_location_label'] if data.get('gps_point') else data['country_display']
        self._run(p, caption, bold=True, size=7.8, color=BLUE_2)
    ...
    def _build_annex_pages(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for start in range(0, len(images), 6):
            ...
            card_height = 2.85 if rows > 1 else 4.90
            ...
            for idx, path in enumerate(chunk):
                ...
                pic_px = (900, 600) if cols <= 2 else (760, 430)
                picture = self._prepare_photo(path, self._temp_dir / f'annex_{start + idx}.png', pic_px) if self._temp_dir else None
                p = self._cell_p(c)
    ...
    def _make_icon(self, kind: str, sector: bool = False) -> Path:
        ...
        d = ImageDraw.Draw(im)
        blue = (*_pil_rgb(BLUE_2), 255)
        pale = (*_pil_rgb(PALE_BLUE), 255)
        orange = (*_pil_rgb(ORANGE), 255)
        d.ellipse((12, 12, size - 12, size - 12), fill=pale)
        if sector:
            self._draw_sector_icon_pil(d, kind, (90, 90), blue, scale=1.0)
            d.ellipse((137, 134, 158, 155), fill=orange)
        else:
            self._draw_line_icon_pil(d, kind, (90, 90), blue, scale=1.0)
        im.save(path)
        self._icon_cache[key] = path
        return path

    def _prepare_photo(self, source: Path | None, output: Path, size: tuple[int, int]) -> Path | None:
        if source and Path(source).exists():
            try:
                ...
                pass
        # non-empty placeholder, never a blank box
        ...
        text = 'Visuel non disponible'
        tw = self._text_w(d, text, font)
        d.text(((size[0] - tw) / 2, size[1] / 2 - 15), text, fill='#6B7280', font=font)
        im.save(output, quality=92)
        return output
    ...
    def _render_map_image(self, data: dict[str, Any], output: Path, size: tuple[int, int], lang: str) -> Path:
        ...
        shape = self._country_shape(data['country'])
        if not shape:
            draw.rounded_rectangle((1, 1, width - 2, height - 2), radius=20, outline='#D4DAE3', fill='#FFFFFF')
        ...
        bbox_expanded = (minx - dx * 0.28, miny - dy * 0.24, maxx + dx * 0.28, maxy + dy * 0.24)
        ...
        for sr in self._iter_shapes():
            ...
            for poly in s_polys:
                pts = [mapper(lon, lat) for lon, lat in poly]
                if len(pts) >= 3:
                    draw.polygon(pts, fill='#FFFFFF' if is_target else '#F1F3F6', outline='#C9D1DD' if is_target else '#E0E5ED')
        # country and neighbor labels
        country_font = self._font(34, bold=True)
        cx, cy = mapper((minx + maxx) / 2, (miny + maxy) / 2)
        label = data['country_display'].upper()
        draw.text((cx - self._text_w(draw, label, country_font) / 2, cy - 18), label, fill='#6B7280', font=country_font)
        ...
        for name, lon, lat in labels[:6]:
            ...
            draw.text((lx - self._text_w(draw, txt, label_font) / 2, ly - 8), txt, fill='#9AA2AE', font=label_font)
        point = data.get('gps_point')
        if point:
            ...
            if self._point_in_bbox(lon, lat, bbox_expanded):
                ...
                draw.text((px + 22, py - 16), pin_label or data['location'], fill='#1D2D44', font=self._font(18, bold=True))
        draw.rounded_rectangle((0, 0, width - 1, height - 1), radius=20, outline='#D4DAE3', width=3)
        im.save(output, quality=95)
        return output
    ...
    def _narrative(self, activity: str, project_name: str, description: str, quantity: dict[str, str], location: str, lang: str) -> str:
        ...
        label = quantity['label'].lower()
        act = activity[:1].lower() + activity[1:] if activity else 'activité terrain'
        if lang == 'fr':
            parts = [f"Cette activité de {act} a été documentée à {location} dans le cadre du projet {project_name}."]
            if description:
                parts.append(f"Les informations disponibles indiquent : {description}.")
            if value and label:
                if 'kit' in label:
                    parts.append(f"Le dossier met en évidence {value} kits comme volume principal de l’activité.")
                elif 'abri' in label:
                    parts.append(f"Le dossier met en évidence {value} abris comme volume principal de l’activité.")
                else:
                    parts.append(f"Le dossier met en évidence {value} {label} comme chiffre clé de l’activité.")
            parts.append("Les éléments collectés apportent un support visuel utile pour illustrer la mise en œuvre et renforcer la traçabilité du reporting bailleur, sans aller au-delà des informations fournies.")
        ...
        if description:
            parts.append(f"Available information states: {description}.")
        if value and label:
            parts.append(f"The file highlights {value} {label} as the key figure for this activity.")
        parts.append('The collected elements provide visual support for implementation reporting without going beyond the information provided.')
        return ' '.join(_sentence(p) for p in parts)
    ...
    def _highlights(self, activity: str, project_code: str, description: str, quantity: dict[str, str], media_count: int, lang: str) -> list[str]:
        act = activity[:1].lower() + activity[1:] if activity else 'activité terrain'
        if lang == 'fr':
            highlights = [f"Une activité de {act} a été documentée dans le cadre du projet {project_code}."]
            if quantity['value'] and quantity['label']:
                highlights.append(f"Le dossier retient {quantity['value']} {quantity['label'].lower()} comme chiffre clé de l’activité, sur la base des informations source.")
            if media_count > 0:
        ...
        highlights = [f"A {act} activity was documented under project {project_code}."]
        if quantity['value'] and quantity['label']:
            highlights.append(f"The file retains {quantity['value']} {quantity['label'].lower()} as the key figure for the activity, based on source information.")
        if media_count > 0:
