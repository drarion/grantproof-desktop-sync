...
HUMANITARIAN_ICON_MAP = {
    ...
    'cash': 'cash_transfer.png',
    'coordination': 'cccm.png',
    'people': 'beneficiaries.png',
    'beneficiary': 'beneficiaries.png',
    'beneficiaries': 'beneficiaries.png',
    'group': 'groups.png',
    'groups': 'groups.png',
    'kits': 'kits.png',
    'box': 'kits.png',
...
REPORT_ICON_MAP = {
    ...
    'cash_assistance': 'cash_transfer',
    'voucher': 'cash_transfer',
    ...
    'section_logframe': 'section_logframe',
    'section_sector': 'section_sector',
    'section_highlights': 'section_highlights',
    'section_location': 'section_location',
}

ACTIVITY_ICON_KEYWORDS = [
    ('cash_transfer', ['cash', 'transfert monétaire', 'transferts monétaires', 'voucher', 'coupon']),
    ('kits', ['kit', 'kits', 'distribution de kits', 'kits agricoles']),
    ('distribution', ['distribution', 'remise', 'dotation']),
    ('construction', ['construction', 'construire', 'bâtiment', 'ouvrage']),
    ('shelter', ['abri', 'abris', 'shelter', 'logement']),
    ('training', ['formation', 'atelier', 'session', 'renforcement de capacités']),
    ('agriculture', ['agricole', 'agriculture', 'maraîcher', 'maraicher', 'semences', 'intrants']),
    ('food_security', ['sécurité alimentaire', 'vivres', 'ration', 'nutrition sensible']),
    ('wash', ['eau', 'wash', 'eha', 'latrine', 'assainissement', 'hygiène']),
    ('health', ['santé', 'clinique', 'médical', 'soins', 'vaccination']),
    ('education', ['école', 'éducation', 'scolaire', 'enseignant', 'élève']),
    ('protection', ['protection', 'vbg', 'gbv', 'enfant', 'psychosocial']),
    ('livestock', ['bétail', 'élevage', 'livestock', 'caprin', 'ovin']),
    ('irrigation', ['irrigation', 'forage', 'pompe', 'arrosage']),
    ('market', ['marché', 'market', 'commercialisation']),
]
...
class PremiumActivityReportBuilder:
    ...
    def build(self, output: Path, project_code: str, project_name: str, records: list[Any], lang: str) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix='grantproof_report_assets_') as tmp:
            ...
            data['dashboard_picture'] = self._render_dashboard_page_image(data, self._temp_dir / 'dashboard_page1.png', lang)
            doc = Document()
            self._configure_docx(doc)
            self._build_page_one_image(doc, data, lang)
    ...
    # ------------------------------------------------------------------ image-based premium dashboard page

    def _build_page_one_image(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        p = doc.add_paragraph()
        self._pconf(p, before=0, after=0, line=0.1)
        p.paragraph_format.left_indent = Inches(0)
        p.paragraph_format.right_indent = Inches(0)
        p.add_run().add_picture(str(data['dashboard_picture']), width=Inches(11), height=Inches(8.25))

    def _render_dashboard_page_image(self, data: dict[str, Any], output: Path, lang: str) -> Path:
        W, H = 1448, 1086
        ...
        # Fonts chosen to visually match the premium model (condensed, NGO-style headings)
        def f(size: int, bold: bool = False, condensed: bool = False):
            candidates = []
            if condensed:
                candidates += [
                    '/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Bold.ttf' if bold else '/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Regular.ttf',
        ...
        font_title = f(44, True, True)
        font_subtitle = f(23, False, True)
        font_sec = f(26, True, True)
        font_body = f(18, False, False)
        font_body_bold = f(18, True, False)
        font_small = f(14, False, False)
        font_small_bold = f(14, True, False)
        font_kpi = f(56, True, True)
        font_kpi_label = f(18, False, False)

        # Header full bleed, text respects body margins
        header_h = 156
        d.rectangle((0, 0, W, header_h), fill=blue)
        body_l, body_r = 42, W - 42
        # title wrapper to avoid overflow while matching one-line visual when possible
        title = data['title'].upper()
        self._draw_fit_text(d, title, (body_l, 38, 960, 84), font_title, '#FFFFFF')
        prefix = 'Projet' if lang == 'fr' else 'Project'
        d.text((body_l, 101), f"{prefix} : {data['project_name']} ({data['project_code']})", font=font_subtitle, fill='#FFFFFF')
        # Header map shifted left from the meta block
        country = Image.open(data['header_country_picture']).convert('RGBA')
        country.thumbnail((134, 90), Image.Resampling.LANCZOS)
        im.paste(country, (1048, 31), country)
        d.line((1228, 24, 1228, 136), fill='#D7E7FF', width=2)
        meta_x = 1262
        icon_font = self._fa_font(27)
        fa = {'org':'\uf007', 'pin':'\uf041', 'cal':'\uf073'}
        meta_values = [data['org_name'], data['location'], data['report_date']]
        for idx, (glyph, value) in enumerate(zip([fa['org'], fa['pin'], fa['cal']], meta_values)):
            y = 30 + idx * 39
            d.text((meta_x, y), glyph, font=icon_font, fill='#FFFFFF')
            d.text((meta_x + 42, y - 1), self._ellipsize(d, value, f(22, False, True), 145), font=f(22, False, True), fill='#FFFFFF')

        # Main layout coordinates copied from the premium model proportions
        y_top = 178
        # Overview card
        self._draw_card(d, (42, y_top, 453, 736), radius=14, fill='#FFFFFF', outline=border, shadow=True)
        self._draw_section_heading(im, d, (66, y_top + 19), 'APERÇU DE L’ACTIVITÉ' if lang == 'fr' else 'ACTIVITY OVERVIEW', 'section_overview', 56, model=True)
        summary = data.get('summary') or ''
        self._draw_wrapped_text(d, summary, (62, y_top + 82, 405, y_top + 365), font_body, dark, line_spacing=7)
        d.line((62, y_top + 375, 428, y_top + 375), fill='#E5ECF6', width=2)
        # Metadata rows lower, never clipped
        meta_rows = [
        ...
        for icon, label, value in meta_rows:
            self._paste_icon(im, icon, (67, y + 2), 42, blue2)
            d.text((122, y), label, font=font_body_bold, fill=blue2)
            self._draw_wrapped_text(d, value, (122, y + 24, 420, y + 55), font_body, dark, line_spacing=2)
            y += 61
            if y < y_top + 565:
                d.line((62, y - 9, 428, y - 9), fill='#E5ECF6', width=2)

        # KPI heading and cards
        self._draw_section_heading(im, d, (492, y_top + 22), 'CHIFFRES CLÉS' if lang == 'fr' else 'KEY FIGURES', 'section_kpis', 48, model=False)
        d.line((548, y_top + 62, 958, y_top + 62), fill='#7CA2D3', width=2)
        kpis = list(data.get('kpis') or [])[:3]
        while len(kpis) < 3:
            if len(kpis) == 0:
                kpis.append({'value': data.get('quantity', {}).get('value', '1'), 'label': data.get('quantity', {}).get('label', 'Preuve'), 'icon':'beneficiaries'})
            elif len(kpis) == 1:
                kpis.append({'value': str(data.get('media_count', 0)), 'label': 'Médias liés' if lang == 'fr' else 'Linked media', 'icon':'media'})
            else:
                kpis.append({'value': str(data.get('evidence_count', 1)), 'label': 'Preuve consolidée' if lang == 'fr' else 'Evidence', 'icon':'check'})
        kpi_xs = [486, 648, 810]
        for x, kpi in zip(kpi_xs, kpis):
            self._draw_card(d, (x, y_top + 122, x + 145, y_top + 520), radius=13, fill='#FFFFFF', outline='#E5EAF1', shadow=True)
            # pale icon disk like model
            d.ellipse((x + 34, y_top + 163, x + 111, y_top + 240), fill='#F0F4FA')
            self._paste_icon(im, kpi.get('icon','people'), (x + 52, y_top + 181), 42, blue2)
            val = str(kpi.get('value',''))
            tw = self._text_w(d, val, font_kpi)
            d.text((x + 72 - tw/2, y_top + 300), val, font=font_kpi, fill=blue2)
            d.line((x + 60, y_top + 381, x + 85, y_top + 381), fill=orange, width=3)
            label = str(kpi.get('label',''))
            self._draw_centered_wrapped(d, label, (x + 12, y_top + 405, x + 133, y_top + 470), font_kpi_label, dark)

        # Hero photo with rounded edges
        hero = Image.open(data['hero_picture']).convert('RGB')
        hero = self._cover_crop(hero, (420, 365))
        hero = self._round_image(hero, 14)
        im.paste(hero, (1000, y_top + 2), hero)

        # Sector alignment block
        self._draw_section_heading(im, d, (1005, y_top + 386), 'ALIGNEMENT SECTORIEL' if lang == 'fr' else 'SECTOR ALIGNMENT', 'section_sector', 46, model=True)
        d.line((1066, y_top + 429, 1404, y_top + 429), fill='#7CA2D3', width=2)
        sectors = data.get('sectors', [])[:2] or [{'icon':'coordination','label':'Coordination'}]
        sector_y = y_top + 470
        if len(sectors) == 1:
            sector_positions = [1188]
        else:
            sector_positions = [1113, 1287]
            d.line((1200, sector_y - 5, 1200, sector_y + 118), fill='#DCE6F1', width=2)
        for cx, sector in zip(sector_positions, sectors):
            self._paste_icon(im, sector.get('icon','activity'), (cx - 33, sector_y), 66, blue2)
            self._draw_centered_wrapped(d, sector.get('label',''), (cx - 62, sector_y + 80, cx + 62, sector_y + 126), font_body, dark)

        # Divider line between top and bottom sections
        d.line((42, 750, 1406, 750), fill='#E5ECF6', width=2)

        # Bottom highlights card
        self._draw_card(d, (42, 760, 705, 1056), radius=12, fill=soft, outline='#E5ECF6', shadow=False)
        self._draw_section_heading(im, d, (66, 782), 'FAITS SAILLANTS' if lang == 'fr' else 'HIGHLIGHTS', 'section_highlights', 54, model=True)
        d.line((128, 822, 526, 822), fill='#7CA2D3', width=2)
        hy = 854
        for item in (data.get('highlights') or [])[:3]:
            d.ellipse((64, hy + 7, 73, hy + 16), fill=blue2)
            self._draw_wrapped_text(d, item, (92, hy, 660, hy + 58), font_body, dark, line_spacing=4)
            hy += 58

        # Bottom location card
        self._draw_card(d, (725, 760, 1406, 1056), radius=12, fill=soft, outline='#E5ECF6', shadow=False)
        self._draw_section_heading(im, d, (755, 782), 'LOCALISATION' if lang == 'fr' else 'LOCATION', 'section_location', 54, model=True)
        d.line((820, 822, 938, 822), fill='#7CA2D3', width=2)
        d.text((755, 912), 'Localisation de l’activité' if lang == 'fr' else 'Activity location', font=font_body_bold, fill=blue2)
        d.line((755, 942, 825, 942), fill=orange, width=3)
        loc_text = data.get('location','')
        d.text((755, 970), self._ellipsize(d, loc_text, font_body, 180), font=font_body, fill=dark)
        if data.get('gps_point'):
            lat, lon, _ = data['gps_point']
            d.text((755, 995), f'GPS : {lat:.4f}, {lon:.4f}', font=font_small, fill=muted)
        map_img = Image.open(data['map_picture']).convert('RGB')
        # Soft map, no heavy border, very close to model
        map_img = ImageOps.contain(map_img, (382, 232), Image.Resampling.LANCZOS)
        map_canvas = Image.new('RGB', (410, 235), '#FFFFFF')
        map_canvas.paste(map_img, ((410-map_img.width)//2, (235-map_img.height)//2))
        map_rgba = self._round_image(map_canvas, 10)
        im.paste(map_rgba, (970, 780), map_rgba)
        caption = data.get('map_location_label') or data.get('country_display') or ''
        tw = self._text_w(d, caption, font_body_bold)
        d.text((1175 - tw/2, 1020), caption, font=font_body_bold, fill=blue2)

        im.save(output, quality=95)
        return output

    def _fa_font(self, size: int):
        for candidate in ['/usr/share/fonts/truetype/font-awesome/fontawesome-webfont.ttf', '/usr/share/fonts/opentype/font-awesome/FontAwesome.otf']:
            if Path(candidate).exists():
                return ImageFont.truetype(candidate, size)
        return ImageFont.load_default()

    def _draw_card(self, draw, box, radius=12, fill='#FFFFFF', outline='#D9E4F2', shadow=False):
        x1, y1, x2, y2 = box
        if shadow:
            shadow_layer = Image.new('RGBA', (1448, 1086), (255,255,255,0))
            sd = ImageDraw.Draw(shadow_layer)
            sd.rounded_rectangle((x1+3, y1+4, x2+3, y2+4), radius=radius, fill=(0,0,0,20))
            # shadow compositing is handled indirectly by drawing a soft background rectangle on the main image through paste not available here
            # keep a clean subtle outline instead to avoid over-darkening in Word export
        draw.rounded_rectangle(box, radius=radius, fill=fill, outline=outline, width=1)

    def _draw_section_heading(self, canvas, draw, pos, title, icon_key, icon_size=48, model=True):
        x, y = pos
        blue2 = '#0A57B5'
        if model:
            draw.ellipse((x, y, x + icon_size, y + icon_size), fill=blue2)
            icon = self._icon_as_mono(icon_key, (255,255,255,255), max(28, icon_size-20))
            canvas.paste(icon, (x + (icon_size-icon.width)//2, y + (icon_size-icon.height)//2), icon)
            tx = x + icon_size + 16
        else:
            icon = self._icon_as_mono(icon_key, (10,87,181,255), icon_size)
            canvas.paste(icon, (x, y), icon)
            tx = x + icon_size + 12
        font = ImageFont.truetype('/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Bold.ttf', 26) if Path('/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Bold.ttf').exists() else ImageFont.load_default()
        draw.text((tx, y + (icon_size-28)/2), title, font=font, fill=blue2)

    def _icon_as_mono(self, kind: str, color: tuple[int,int,int,int], size: int):
        icon_path = self._plain_icon(kind)
    ...
    def _paste_icon(self, canvas, kind: str, pos: tuple[int,int], size: int, color: str):
        rgb = tuple(int(color.strip('#')[i:i+2], 16) for i in (0,2,4)) + (255,)
        icon = self._icon_as_mono(kind, rgb, size)
        canvas.paste(icon, pos, icon)

    def _round_image(self, img: Image.Image, radius: int):
    ...
    def _draw_wrapped_text(self, draw, text: str, box, font, fill, line_spacing=5, max_lines=None):
        x1, y1, x2, y2 = box
        ...
        for word in words:
            ...
            if self._text_w(draw, test, font) <= x2 - x1:
                line = test
            else:
                if line:
                    lines.append(line)
        ...
        if line:
            lines.append(line)
        line_h = self._text_h(draw, 'Ag', font) + line_spacing
        y = y1
        count = 0
        for line in lines:
    ...
    def _draw_centered_wrapped(self, draw, text, box, font, fill):
        x1,y1,x2,y2 = box
        words=str(text).split()
        lines=[]; line=''
        for word in words:
            ...
            if self._text_w(draw,test,font)<=x2-x1:
                ...
                line=word
        if line: lines.append(line)
        line_h=self._text_h(draw,'Ag',font)+3
        y=y1
        for line in lines[:3]:
            tw=self._text_w(draw,line,font)
            draw.text((x1+(x2-x1-tw)/2,y),line,font=font,fill=fill)
            y+=line_h

    def _draw_fit_text(self, draw, text, box, font, fill):
        x1,y1,x2,y2=box
        max_w=x2-x1
        ...
        for word in words:
            ...
            if self._text_w(draw,test,font)<=max_w:
                line1=test
            else:
                line2=' '.join(words[words.index(word):])
                break
        draw.text((x1,y1-6),line1,font=font,fill=fill)
        small=ImageFont.truetype('/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Bold.ttf', 36) if Path('/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Bold.ttf').exists() else font
        draw.text((x1,y1+34),self._ellipsize(draw,line2,small,max_w),font=small,fill=fill)

    def _ellipsize(self, draw, text, font, max_w):
        text=str(text)
        if self._text_w(draw,text,font)<=max_w:
            return text
        ell='…'
        while text and self._text_w(draw,text+ell,font)>max_w:
            text=text[:-1]
        return text+ell

    def _text_h(self, draw, text, font):
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_row_height(bottom.rows[0], Inches(1.86))
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        self._cell_margins(meta_cell, top=10, bottom=10, start=10, end=18)
        values = [
            self._truncate_text(data['org_name'], 22),
            self._truncate_text(data['location'], 22),
            data['report_date'],
        ]
        for idx, value in enumerate(values):
            p = self._cell_p(meta_cell) if idx == 0 else meta_cell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.RIGHT, after=0, line=0.9)
            self._run(p, value, size=7.2, color=WHITE)

    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        self._shade_cell(cell, LIGHT_GREY)
        self._border_cell(cell, BORDER)
        self._section_title(cell, 'APERÇU DE L’ACTIVITÉ' if lang == 'fr' else 'ACTIVITY OVERVIEW', 'doc')
        p = cell.add_paragraph()
        self._pconf(p, before=0, line=1.04, after=3)
        self._run(p, data['summary'], size=6.55)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            self._set_row_height(row, Inches(0.36))
            ico, txt = row.cells
            self._set_cell_width(ico, Inches(0.34))
            self._set_cell_width(txt, Inches(2.56))
            self._cell_margins(ico, top=2, bottom=0, start=0, end=16)
            ...
            p = self._cell_p(ico)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.16))
            p = self._cell_p(txt)
            self._pconf(p, line=0.95, after=0)
            self._run(p, label + ' : ', bold=True, size=6.2, color=BLUE_2)
            self._run(p, self._truncate_text(value, 52), size=6.2)
    ...
    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(1.72))
        for kcell in grid.rows[0].cells:
            self._set_cell_width(kcell, Inches(1.32))
            self._cell_margins(kcell, top=12, bottom=6, start=12, end=12)
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._soft_icon(kpi.get('icon', 'people'))), width=Inches(0.34))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.9)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=13.8, color=BLUE_2)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=0.92)
            self._run(p, self._truncate_text(str(kpi.get('label', '')), 24), size=6.6, color=MUTED)
    ...
    def _build_project_logic(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if not activity and not output:
            return
        self._spacer_cell(cell, 2)
        title = 'INTÉGRATION AU CADRE LOGIQUE DU PROJET' if lang == 'fr' else 'PROJECT LOGFRAME INTEGRATION'
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=4, line=0.95)
        self._run(p, title, bold=True, size=7.8, color=BLUE_2)
        if activity:
            p = cell.add_paragraph()
            self._pconf(p, after=2, line=0.95)
            self._run(p, 'Activité' if lang == 'fr' else 'Activity', bold=True, size=7.4, color=ORANGE)
            p = cell.add_paragraph()
            self._pconf(p, line=1.0, after=3)
            p.paragraph_format.left_indent = Inches(0.26)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=7.2, color=DARK)
            self._run(p, self._truncate_text(activity, 86), size=7.2)
        if output:
            p = cell.add_paragraph()
            self._pconf(p, after=2, line=0.95)
            self._run(p, 'Output' if lang == 'fr' else 'Output', bold=True, size=7.4, color=ORANGE)
            p = cell.add_paragraph()
            self._pconf(p, line=1.0, after=0)
            p.paragraph_format.left_indent = Inches(0.26)
            p.paragraph_format.first_line_indent = Inches(-0.12)
            self._run(p, '• ', size=7.2, color=DARK)
            self._run(p, self._truncate_text(output, 116), size=7.2)
    ...
    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(3.35), height=Inches(1.70))
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            ...
            self._cell_margins(scell, top=6, bottom=0, start=4, end=4)
            p = self._cell_p(scell)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=2)
            p.add_run().add_picture(str(self._sector_icon(sector['icon'])), width=Inches(0.64))
    ...
    def _build_annex_pages(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for start in range(0, len(images), 6):
            ...
            p.paragraph_format.right_indent = Inches(0.70)
    ...
    def _report_icon_asset(self, kind: str, style: str = 'plain') -> Path | None:
        ...
        if not source.exists():
            source = self.assets_dir / 'report_icons' / 'activity.png'
        if not source.exists():
            return None
        ...
        max_box = (188, 188) if style == 'badge' else (212, 212)
        im.thumbnail(max_box, Image.Resampling.LANCZOS)
        if style in {'white', 'badge'}:
            pixels = im.load()
        ...
        if style == 'badge':
            ...
            d.ellipse((18, 18, 238, 238), fill=(*_pil_rgb(BLUE_2), 255))
    ...
    def _prepare_photo(self, source: Path | None, output: Path, size: tuple[int, int], fit: str = 'cover') -> Path | None:
        if source and Path(source).exists():
            try:
                ...
                d = ImageDraw.Draw(im)
                d.rounded_rectangle((1, 1, size[0] - 2, size[1] - 2), radius=20, outline='#D4DAE3', width=3)
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '──────────') -> None:
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=1, line=0.95)
        if icon:
            p.add_run().add_picture(str(self._badge_icon(icon)), width=Inches(0.18))
            self._run(p, '  ', size=1)
        self._run(p, title, bold=True, size=8.2, color=BLUE_2)

    def _spacer_cell(self, cell, points: float = 2) -> None:
        p = cell.add_paragraph()
        self._pconf(p, after=points, line=0.2)
