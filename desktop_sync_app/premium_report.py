...
REPORT_ICON_MAP = {
    ...
    'model_kpi_beneficiaries': 'model_kpi_beneficiaries',
    'model_kpi_media': 'model_kpi_media',
    'model_kpi_evidence': 'model_kpi_evidence',
    'model_kpi_kits': 'model_kpi_kits',
    'model_kpi_groups': 'model_kpi_groups',
    'model_kpi_cash': 'model_kpi_cash',
    'model_kpi_shelter': 'model_kpi_shelter',
    'model_kpi_training': 'model_kpi_training',
    'model_info_location': 'model_info_location',
    'model_info_people': 'model_info_people',
    'model_info_photo': 'model_info_photo',
    'model_sector_food': 'model_sector_food',
    'model_sector_agriculture': 'model_sector_agriculture',
}
...
class PremiumActivityReportBuilder:
    ...
    def _build_page_one(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        top.autofit = False
        self._set_row_height(top.rows[0], Inches(4.65))
        widths = [3.25, 4.00, 3.35]
        for idx, (cell, width) in enumerate(zip(top.rows[0].cells, widths)):
            self._set_cell_width(cell, Inches(width))
            margins = [(26, 30), (32, 30), (28, 26)][idx]
            self._cell_margins(cell, top=18, bottom=12, start=margins[0], end=margins[1])
        ...
        self._set_row_height(bottom.rows[0], Inches(1.55))
        for idx, (cell, width) in enumerate(zip(bottom.rows[0].cells, [5.15, 5.45])):
            self._set_cell_width(cell, Inches(width))
            self._cell_margins(cell, top=22, bottom=18, start=34, end=34)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.TOP
    ...
    def _build_header(self, doc: Document, data: dict[str, Any], lang: str) -> None:
        ...
        for idx, (cell, width) in enumerate(zip(table.rows[0].cells, [8.35, 1.05, 1.60])):
            self._set_cell_width(cell, Inches(width))
            self._shade_cell(cell, BLUE)
            if idx == 0:
                self._cell_margins(cell, top=26, bottom=22, start=28, end=20)
            elif idx == 1:
                self._cell_margins(cell, top=16, bottom=16, start=0, end=8)
            else:
                self._cell_margins(cell, top=24, bottom=20, start=8, end=42)
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        title_cell, map_cell, meta_cell = table.rows[0].cells
        p = self._cell_p(title_cell)
        self._pconf(p, after=2, line=0.94)
        title_size = 12.8 if len(data['title']) < 90 else 11.0
        self._run(p, data['title'].upper(), bold=True, size=title_size, color=WHITE, font='Liberation Sans Narrow')
        p = title_cell.add_paragraph()
        self._pconf(p, after=0, line=0.98)
        prefix = 'Projet' if lang == 'fr' else 'Project'
        self._run(p, f"{prefix} : {data['project_name']} ({data['project_code']})", size=8.8, color=WHITE)
        ...
        values = [
            self._truncate_text(data['org_name'], 24),
            self._truncate_text(data['location'], 24),
            data['report_date'],
        ]
        for idx, value in enumerate(values):
            p = self._cell_p(meta_cell) if idx == 0 else meta_cell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.RIGHT, after=0, line=1.05)
            p.paragraph_format.right_indent = Inches(0.05)
            self._run(p, value, size=10.0, color=WHITE)

    def _build_overview(self, cell, data: dict[str, Any], lang: str) -> None:
        self._shade_cell(cell, WHITE)
        self._border_cell(cell, BORDER_LIGHT)
        self._section_title(cell, 'APERÇU DE L’ACTIVITÉ' if lang == 'fr' else 'ACTIVITY OVERVIEW', 'model_section_overview', underline_width=2.65)
        p = cell.add_paragraph()
        self._pconf(p, before=5, line=1.0, after=4)
        self._run(p, data['summary'], size=8.0, color=DARK)
        self._spacer_cell(cell, 6)
        ...
        for row, (icon, label, value) in zip(info.rows, rows):
            ico, txt = row.cells
            self._set_cell_width(ico, Inches(0.42))
            self._set_cell_width(txt, Inches(2.45))
            self._cell_margins(ico, top=2, bottom=2, start=0, end=14)
            self._cell_margins(txt, top=2, bottom=2, start=0, end=0)
            ico.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            txt.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            p = self._cell_p(ico)
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.26))
            p = self._cell_p(txt)
            self._pconf(p, line=1.05, after=0)
            self._run(p, label, bold=True, size=8.0, color=BLUE_2)
            p = txt.add_paragraph()
            self._pconf(p, line=1.05, after=0)
            self._run(p, value, size=8.0, color=DARK)
    ...
    def _kpi_icon_key(self, kpi: dict[str, Any]) -> str:
        ...
        if any(term in joined for term in ['cash', 'voucher']):
            return 'model_kpi_cash'
        if any(term in joined for term in ['shelter', 'abri']):
            return 'model_kpi_shelter'
        if any(term in joined for term in ['training', 'formation']):
            return 'model_kpi_training'
        return 'model_kpi_evidence'

    def _build_kpis(self, cell, data: dict[str, Any], lang: str) -> None:
        self._section_title(cell, 'CHIFFRES CLÉS' if lang == 'fr' else 'KEY FIGURES', 'model_section_kpis', underline_width=3.4)
        kpis = list(data.get('kpis') or [])
        normalized = []
        seen = set()
        preferred_order = ['beneficiary', 'beneficiaries', 'people', 'kit', 'kits', 'group', 'groups', 'media', 'evidence']
        while len(kpis) < 3:
        ...
        grid.autofit = False
        self._set_row_height(grid.rows[0], Inches(1.78))
        for kcell in grid.rows[0].cells:
            self._set_cell_width(kcell, Inches(1.22))
            self._cell_margins(kcell, top=10, bottom=10, start=10, end=10)
        ...
        for idx, kpi in enumerate(normalized[:3]):
            ...
            p.add_run().add_picture(str(self._plain_icon(self._kpi_icon_key(kpi))), width=Inches(0.50))
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=1, line=0.95)
            self._run(p, self._truncate_text(str(kpi.get('value', '')), 14), bold=True, size=18.0, color=BLUE_2)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=1.0)
            self._run(p, '—', bold=True, size=12.0, color=ORANGE)
            p = kcell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0, line=1.05)
            self._run(p, self._truncate_text(str(kpi.get('label', '')), 24), size=9.0, color=DARK)
    ...
    def _build_project_logic(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        if activity:
            ...
            self._run(p, 'Activité : ', bold=True, size=8.0, color=ORANGE)
            self._run(p, activity, size=8.0)
        if output:
            p = cell.add_paragraph()
            self._pconf(p, after=0, line=1.0)
            self._run(p, 'Output : ', bold=True, size=8.0, color=ORANGE)
            self._run(p, output, size=8.0)

    def _build_media_sector(self, cell, data: dict[str, Any], lang: str) -> None:
        p = self._cell_p(cell)
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=12)
        if Path(data['hero_picture']).exists():
            p.add_run().add_picture(str(data['hero_picture']), width=Inches(3.20), height=Inches(2.05))
        ...
        for scell, sector in zip(table.rows[0].cells, sectors):
            self._set_cell_width(scell, Inches(1.48))
            ...
            icon_key = 'model_sector_food' if sector.get('key') in {'food_security', 'food'} else ('model_sector_agriculture' if sector.get('key') == 'agriculture' else sector['icon'])
            p.add_run().add_picture(str(self._sector_icon(icon_key)), width=Inches(0.58))
            p = scell.add_paragraph()
            self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, line=1.0, after=0)
            self._run(p, sector['label'], size=9.0)
    ...
    def _build_highlights(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        for item in data['highlights'][:3]:
            ...
            self._run(p, '• ', bold=True, color=BLUE_2, size=11.0)
            self._run(p, item, size=8.2)
    ...
    def _build_location(self, cell, data: dict[str, Any], lang: str) -> None:
        ...
        self._set_cell_width(left, Inches(2.05))
        self._set_cell_width(right, Inches(3.18))
        self._cell_margins(left, top=0, bottom=0, start=0, end=24)
        self._cell_margins(right, top=0, bottom=0, start=0, end=0)
        self._section_title(left, 'LOCALISATION' if lang == 'fr' else 'LOCATION', 'model_section_location', underline_width=2.05)
        p = left.add_paragraph()
        self._pconf(p, before=18, after=2)
        self._run(p, 'Localisation de l’activité' if lang == 'fr' else 'Activity location', bold=True, size=8.2, color=BLUE_2)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=1.05)
        self._run(p, data['location'], size=8.0, color=DARK)
        p = left.add_paragraph()
        self._pconf(p, after=0, line=1.05)
        if data.get('gps_point'):
            lat, lon, _ = data['gps_point']
            self._run(p, f'GPS : {lat:.4f}, {lon:.4f}', size=8.3, color=MUTED)
        else:
            self._run(p, 'Carte nationale affichée par défaut.', size=8.3, color=MUTED)
        p = self._cell_p(right)
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        p.add_run().add_picture(str(data['map_picture']), width=Inches(3.05), height=Inches(1.62))
        p = right.add_paragraph()
        self._pconf(p, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
        caption = data['map_location_label'] if data.get('gps_point') else data['country_display']
        self._run(p, caption, bold=True, size=8.0, color=BLUE_2)
    ...
    def _section_title(self, cell, title: str, icon: str | None = None, line_chars: str = '──────────', underline_width: float = 2.2) -> None:
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=0, line=0.95)
        if icon:
            p.add_run().add_picture(str(self._plain_icon(icon)), width=Inches(0.28))
            self._run(p, '  ', size=1)
        self._run(p, title, bold=True, size=10.0, color=BLUE_2)
        p = cell.add_paragraph()
        self._pconf(p, before=0, after=2, line=0.42)
        self._run(p, '─' * max(8, int(underline_width * 9)), size=5.0, color=BLUE_3)

    def _cell_p(self, cell):
        return cell.paragraphs[0] if cell.paragraphs else cell.add_paragraph()

    def _run(self, paragraph, text: str, bold: bool = False, size: float = 8.5, color: str = DARK, italic: bool = False, font: str = 'Segoe UI'):
