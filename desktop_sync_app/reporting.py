...
class GrantProofAIWriter:
    ...
    def executive_summary(self, project_name: str, donor_name: str, country: str, records: list[ItemRecord], lang: str) -> list[str]:
        ...
        if lang == 'fr':
            ...
            para1 = (
                f'Ce pack GrantProof consolide les dernières preuves terrain collectées pour {project_name}{country_clause}{donor_clause}. '
                f'La fenêtre actuelle rassemble {len(evidence_records)} preuve(s), {len(story_records)} story(s) et {media_count} média(s) synchronisés, '
                f'ce qui permet de transformer des captures brutes en matière directement mobilisable pour un reporting bailleur de qualité.'
            )
            para2 = (
                f'La dynamique de mise en œuvre la plus visible concerne {top_activity}, avec un signal particulièrement net autour de {top_output}. '
                f'Le portefeuille documenté montre une continuité crédible entre activité réalisée, résultat observable et éléments de vérification visuelle ou narrative.'
            )
            para3 = (
                f'L’empreinte opérationnelle ressort surtout à {top_location}. '
                f'Les descriptions synchronisées mettent en avant {descriptive_signals}, ce qui renforce la valeur du dossier pour le suivi, la communication de résultats et la préparation d’audits.'
            )
            para4 = (
                f'La qualité du matériau disponible est renforcée par {gps_count} élément(s) géolocalisé(s) et par une base média suffisamment structurée pour produire des annexes propres, illustrées et directement réutilisables.'
            )
        else:
            ...
            para1 = (
                ...
                f'The current window brings together {len(evidence_records)} evidence item(s), {len(story_records)} story item(s), and {media_count} synced media asset(s), '
                f'allowing raw field capture to be converted into reporting material that is immediately usable for donor-facing communication.'
            )
            para2 = (
                ...
                f'The documented portfolio shows a credible continuity between activity delivery, observable outputs, and supporting visual or narrative verification.'
            )
            para3 = (
                f'The clearest operational footprint appears in {top_location}. '
                f'Synced descriptions consistently point to {descriptive_signals}, which increases the value of the pack for progress reporting, visibility material, and audit preparation.'
            )
            para4 = (
                f'The available material is further strengthened by {gps_count} geo-referenced item(s) and by a media base robust enough to produce clean, illustrated annexes that can be reused directly in project reporting.'
            )
    ...
    def key_highlights(self, records: list[ItemRecord], lang: str) -> list[str]:
        ...
        if lang == 'fr':
            highlights = [
                ...
                f'{media_rich} élément(s) disposent de média(s) liés, ce qui améliore fortement la valeur de vérification et la qualité des annexes.',
                f'{with_descriptions} élément(s) contiennent un texte descriptif exploitable pour densifier le narratif.',
            ]
        else:
            highlights = [
                ...
                f'{with_descriptions} item(s) contain usable descriptive text that can support denser narrative drafting.',
            ]
        if activity_counts:
            activity, count = activity_counts.most_common(1)[0]
            highlights.append(
                f"L’activité la plus documentée est {activity} avec {count} preuve(s) liées." if lang == 'fr' else f'The most documented activity stream is {activity} with {count} linked evidence item(s).'
            )
        if output_counts:
    ...
    def implementation_summary(self, activity: str, records: list[ItemRecord], lang: str) -> str:
        ...
        if lang == 'fr':
            return _sentence(
                ...
                f'Les pièces les plus représentatives portent notamment sur {evidence_titles or "les éléments synchronisés"}, ce qui donne un socle solide pour un paragraphe bailleur précis, concret et vérifiable.'
            )
        return _sentence(
            ...
            f'The most representative entries include {evidence_titles or "the synced items"}, providing a strong base for a donor-facing paragraph that remains precise, concrete, and verifiable.'
        )
        ...
        if description:
            if lang == 'fr':
                return _sentence(
                    ...
                    f'Dans une logique bailleur, cet élément documente un progrès concret vers {output}{media_clause}, tout en apportant une trace exploitable pour illustrer la mise en œuvre réelle sur le terrain.'
                )
            return _sentence(
                f'{subtitle} captured under {activity} in {location}. {description} '
                f'In donor-facing terms, this entry documents tangible progress toward {output}{media_clause}, while also providing an evidentiary trace that can illustrate real implementation on the ground.'
            )
        if lang == 'fr':
            return _sentence(
                f'{subtitle} capturée dans le cadre de {activity} à {location}. '
                f'Même avec peu de texte libre, cet élément constitue un signal de mise en œuvre vérifiable lié à {output}{media_clause}.'
            )
        return _sentence(
            f'{subtitle} captured under {activity} in {location}. '
            f'Even with limited free-text detail, the item still provides a verifiable implementation signal linked to {output}{media_clause}.'
        )
    ...
    def story_narrative(self, record: ItemRecord, lang: str) -> str:
        ...
        quote_clause = (
            f' La citation enregistrée — « {quote} » — renforce la crédibilité humaine du signal de changement.'
            if lang == 'fr' and quote else
            f' The recorded quote — “{quote}” — reinforces the human credibility of the change signal.' if quote else ''
        )
        if summary:
            if lang == 'fr':
                return _sentence(
                    f'Cette story met en lumière {beneficiary}. {summary} '
                    f'Pour le reporting bailleur, elle complète utilement la trace opérationnelle en donnant une lecture qualitative plus incarnée des résultats observés.{quote_clause}'
                )
            return _sentence(
                f'This story focuses on {beneficiary}. {summary} '
                f'For donor reporting, it usefully complements the operational trail by offering a more human and qualitative view of the observed results.{quote_clause}'
            )
        if lang == 'fr':
            return _sentence(
                f'Cette story met en lumière {beneficiary}. '
                f'Elle apporte une couche qualitative à la base de preuves et aide à traduire la mise en œuvre du projet en récit plus humain.{quote_clause}'
            )
        return _sentence(
            f'This story focuses on {beneficiary}. '
            f'It adds a qualitative layer to the evidence base and helps translate project implementation into a more human-facing narrative.{quote_clause}'
        )
    ...
    def monitoring_note(self, records: list[ItemRecord], lang: str) -> list[str]:
        ...
        if lang == 'fr':
            return [
                _sentence(f'Le pack repose sur {media_count} média(s) synchronisé(s), ce qui offre une base solide pour les annexes et la vérification visuelle.'),
                _sentence(f'{gps_count} preuve(s) incluent des coordonnées GPS, renforçant la traçabilité spatiale du dossier.'),
                _sentence(f'{consent_count} story(s) indiquent un consentement confirmé, ce qui soutient une utilisation plus sûre des récits qualitatifs.'),
            ]
        return [
            _sentence(f'The pack relies on {media_count} synced media asset(s), providing a solid base for annexes and visual verification.'),
            _sentence(f'{gps_count} evidence item(s) include GPS coordinates, strengthening the spatial traceability of the pack.'),
            _sentence(f'{consent_count} story item(s) indicate confirmed consent, supporting safer reuse of qualitative narratives.'),
        ]
...
class GrantProofReportEngine:
    ...
    def ensure_root_files(self) -> None:
        ...
        readme.write_text(
            '\n'.join([
                ...
                '- Project_Report.docx',
                '- Project_Report_FR.docx / Project_Report_EN.docx',
    ...
    def rebuild_project(self, project_code: str, preferred_language: str | None = None) -> None:
        ...
        self._build_project_register(reports_dir / 'Project_Register.xlsx', project_code, project_name, records, report_lang)
        self._build_project_report(reports_dir / 'Project_Report.docx', project_code, project_name, records, report_lang)
        ...
        self._build_project_report(reports_dir / 'Project_Report_FR.docx', project_code, project_name, records, 'fr')
        self._build_project_report(reports_dir / 'Project_Report_EN.docx', project_code, project_name, records, 'en')
        for record in records:
            item_lang = self._record_language(record, fallback=report_lang)
            if record.kind == 'evidence':
                self._build_item_doc(record, 'evidence.docx', item_lang)
            else:
                self._build_item_doc(record, 'story.docx', item_lang)
    ...
    def _build_project_register(self, output: Path, project_code: str, project_name: str, records: list[ItemRecord], lang: str) -> None:
        ...
        for sheet in (overview, narrative, dashboard, timeline, evidence_sheet, story_sheet, media_sheet):
            self._autosize(sheet)
            self._apply_sheet_polish(sheet, auto_filter=True)
    ...
    def _record_showcase(self, document: Document, record: ItemRecord, index: int, lang: str) -> None:
        ...
        if image_files:
            ...
            for cell, image_path in zip(gallery_table.rows[0].cells, image_files[:2]):
                ...
                paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
                try:
                    paragraph.add_run().add_picture(str(image_path), width=Inches(2.65))
                except Exception:
                    paragraph.add_run(image_path.name)
        quote = _safe_text(record.raw.get('quote'))
        if quote:
    ...
    def _project_meta(self, records: list[ItemRecord], preferred_language: str | None = None) -> dict[str, str]:
        ...
        return {
            ...
            'language': self._detect_project_language(records, preferred_language),
        }
    ...
    def _configure_document(self, document: Document) -> None:
        ...
        styles['Normal']._element.rPr.rFonts.set(qn('w:eastAsia'), 'Cambria')
        ...
        styles['Quote'].font.name = 'Cambria'
    ...
    def _header_style(self, cells: Iterable, fill: str = BLUE) -> None:
        for cell in cells:
            cell.font = Font(name='Cambria', bold=True, color='FFFFFF')
    ...
    def _label(self, key: str, lang: str) -> str:
        labels = {
            'fr': {
                ...
                'executive_summary': 'Résumé exécutif', 'key_highlights': 'Points clés pour le bailleur', 'verification_note': 'Note de vérification',
                ...
                'project': 'Projet', 'illustrative_images': 'Galerie visuelle', 'cover_title': 'GrantProof — Rapport projet premium', 'not_specified': 'Non renseigné',
            ...
            'en': {
                ...
                'executive_summary': 'Executive summary', 'key_highlights': 'Key highlights for donor reporting', 'verification_note': 'Verification note',
                ...
                'project': 'Project', 'illustrative_images': 'Visual gallery', 'cover_title': 'GrantProof — Premium project report', 'not_specified': 'Not specified',
